...
public class SampaiTujuanActivity extends AppCompatActivity {
    ...
    private void getData() {
        ...
        apiInterface.sampaiTujuan(id_konsumen).enqueue(new Callback<SampaiTujuanResponse>() {
            @Override
            public void onResponse(Call<SampaiTujuanResponse> call, Response<SampaiTujuanResponse> response) {
                ...
                if (response.isSuccessful()){
                    if (response.body() != null) {
                        if(response.body().getMaster().size()>0){
                            ...
                            LinearLayoutManager manager = new LinearLayoutManager(SampaiTujuanActivity.this);
                            recylerView.setLayoutManager(manager);
                            recylerView.setHasFixedSize(true);
                            transaksiAdapter = new TransaksiAdapter(SampaiTujuanActivity.this, pesananArrayList);
    ...
    //                        Toast.makeText(getApplicationContext(), "Data Menunggu Pembayaran Kosong", Toast.LENGTH_SHORT).show();
            ...
            @Override
            public void onFailure(Call<SampaiTujuanResponse> call, Throwable t) {
